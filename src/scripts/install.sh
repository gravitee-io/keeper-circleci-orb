...
if [ "$UNAME" = "Linux" ]; then
    ...
    echo -e "${ERROR_COLOR}Cannot determine compatible OS type. Exiting...${NO_COLOR}"
    exit;
fi
...
if command -v ksm >/dev/null 2>&1 && ksm version 2>&1 | grep "CLI Version: " | cut -d " " -f 3 | grep -q "$(echo $VERSION)$"; then
